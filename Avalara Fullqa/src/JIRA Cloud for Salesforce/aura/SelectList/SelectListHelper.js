({
    ...
    createOptionPill: function(component, event, helper) {
        ...
        var selectedOptionValues = selectedOptionPills.map(function(x){return x.value});
        if(sourceLabel && selectedOptionValues.indexOf(sourceValue) === -1){
            ...
            if (multiple) {
                ...
                selectedOptionValues.forEach(function(value) {
                    dropDownOptions.forEach(function(option) {
                        if (option.get('v.value') === value) {
                            option.set('v.hidden', false);
                        }
                    });
                });
                window.setTimeout($A.getCallback(function() {
                    component.set('v.selectedOptionPills', [pillAttributes]);
                }), 100);
            }
        }
    ...
    handleValueOnInit: function(component,event,helper){
        ...
        var valueArray = typeof value === "string" ? value.split(';') : [];
        var body = component.get('v.body');
        var childCmps;
        body.forEach(function(child){
            if($A.util.isUndefined(child.filterBy)){
